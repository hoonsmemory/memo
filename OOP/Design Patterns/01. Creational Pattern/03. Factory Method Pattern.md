...
### 사용 목적
1. 관련된 객체들의 생성 : 관련성 있는 여러 객체들을 생성할 수 있다.  
2. 구체적인 클래스 의존성 제거 : 클라이언트 코드가 구체적인 클래스에 의존하지 않도록 하여 결합도를 낮춘다.  
3. 구현체 교체 용이성 : 구현체 전체를 쉽게 교체할 수 있어 시스템의 유연성이 높아진다.  
4. 일관성 유지 : 동일한 팩토리를 통해 생성된 객체들은 서로 호환되며, 제품들 간의 일관성을 유지한다.
<br>
<br>
